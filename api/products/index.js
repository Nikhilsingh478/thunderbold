  ...
  try {
    ...
    switch (req.method) {
      ...
      case 'POST': {
        ...
        const isOutfitSection = section === 'outfits';
        const images = normaliseImages(req.body);
        const needsCategory = section !== 'live-sale' && section !== 'kurta' && !isOutfitSection;
        ...
        let sizeFields;
        if (isOutfitSection) {
          const top = req.body.topwear || {};
          const bot = req.body.bottomwear || {};
          const topSS = normaliseSizeStock(top.sizeStock);
          const botSS = normaliseSizeStock(bot.sizeStock);
          const topStock = computeTotalStock(topSS);
          const botStock = computeTotalStock(botSS);
          sizeFields = {
            topwear: { sizeStock: topSS, stock: topStock, highlights: normaliseHighlights(top.highlights) },
            bottomwear: { sizeStock: botSS, stock: botStock, highlights: normaliseHighlights(bot.highlights) },
            stock: Math.min(topStock, botStock),
          };
        } else {
          ...
          sizeFields = {
            ...
            stock: totalStock,
            highlights: normaliseHighlights(highlights),
          };
        }
      ...
      case 'PUT': {
        ...
        const isPutOutfit = putSection === 'outfits';
        const images = normaliseImages(req.body);
        const putNeedsCategory = putSection !== 'live-sale' && putSection !== 'kurta' && !isPutOutfit;
        ...
        // When product type changes, clean up fields from the other schema
        if (isPutOutfit) {
          // Outfit stores stock in topwear/bottomwear — unset root sizeStock and highlights
          unsetFields.sizeStock = '';
          unsetFields.highlights = '';
        } else {
          // Non-outfit products should not have topwear/bottomwear sub-objects
          unsetFields.topwear = '';
          unsetFields.bottomwear = '';
        }
