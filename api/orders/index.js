...
async function handleCreate(req, res) {
  ...
  for (const item of products) {
    ...
    const isOutfitProduct = dbProduct.section === "outfits" && dbProduct.topwear && dbProduct.bottomwear;

    if (isOutfitProduct) {
      const topSize = item.topwearSize;
      const botSize = item.bottomwearSize;
      if (!topSize || !botSize) {
        return res.status(400).json({ error: `Outfit "${item.name}" requires both topwear and bottomwear sizes` });
      }
      const topAvail = dbProduct.topwear.sizeStock?.[topSize] ?? 0;
      const botAvail = dbProduct.bottomwear.sizeStock?.[botSize] ?? 0;
      if (topAvail < item.quantity) {
        return res.status(400).json({
          error: topAvail === 0
            ? `"${item.name}" top size ${topSize} is out of stock`
            : `Only ${topAvail} unit(s) of "${item.name}" top size ${topSize} available`,
        });
      }
      if (botAvail < item.quantity) {
        return res.status(400).json({
          error: botAvail === 0
            ? `"${item.name}" bottom size ${botSize} is out of stock`
            : `Only ${botAvail} unit(s) of "${item.name}" bottom size ${botSize} available`,
        });
      }
    } else {
      let available;
  ...
  for (const item of products) {
    ...
    const isOutfitDecrement = dbProduct?.section === "outfits" && dbProduct?.topwear && dbProduct?.bottomwear;

    let updateFilter, updateOp;
    if (isOutfitDecrement) {
      const topSize = item.topwearSize;
      const botSize = item.bottomwearSize;
      updateFilter = {
        _id: productObjectId,
        [`topwear.sizeStock.${topSize}`]: { $gte: item.quantity },
        [`bottomwear.sizeStock.${botSize}`]: { $gte: item.quantity },
      };
      updateOp = {
        $inc: {
          [`topwear.sizeStock.${topSize}`]: -item.quantity,
          "topwear.stock": -item.quantity,
          [`bottomwear.sizeStock.${botSize}`]: -item.quantity,
    ...
    const hasSizeStockFallback = !isOutfitDecrement && dbProduct?.sizeStock && typeof dbProduct.sizeStock === "object" && item.size in dbProduct.sizeStock;
    decremented.push({
      id: productObjectId, quantity: item.quantity,
      isOutfit: isOutfitDecrement,
      topwearSize: item.topwearSize,
      bottomwearSize: item.bottomwearSize,
      size: item.size,
      hasSizeStock: hasSizeStockFallback,
    });
  }
  ...
  if (stockError) {
    for (const prev of decremented) {
      ...
      if (prev.isOutfit) {
        restoreOp = {
          $inc: {
            [`topwear.sizeStock.${prev.topwearSize}`]: prev.quantity,
            "topwear.stock": prev.quantity,
            [`bottomwear.sizeStock.${prev.bottomwearSize}`]: prev.quantity,
            "bottomwear.stock": prev.quantity,
            stock: prev.quantity,
          },
        };
      } else {
        restoreOp = prev.hasSizeStock
...
async function handleCancel(req, res) {
  ...
  for (const item of orderProducts) {
    ...
    try {
      ...
      const isOutfitRestore = dbProduct?.section === "outfits" && dbProduct?.topwear && dbProduct?.bottomwear && item.topwearSize && item.bottomwearSize;
