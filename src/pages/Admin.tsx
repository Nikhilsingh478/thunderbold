...
function getSizesForSection(section: string): string[] {
  return (section === 'tshirts' || section === 'kurta' || section === 'outfits')
    ? [...APPAREL_SIZES]
    : [...JEANS_SIZES];
}
...
const SECTIONS = [
  ...
  { value: 'outfits', label: 'Thunder Looks' },
] as const;
...
interface ProductFormData {
  ...
  topwearSizeStock: Record<string, string>;
  topwearHighlights: ProductHighlights;
  bottomwearSizeStock: Record<string, string>;
  bottomwearHighlights: ProductHighlights;
}
...
const defaultFormData: ProductFormData = {
  name: '', section: 'denim', categoryId: '', mrp: '', purchasePrice: '', price: '', brandId: '',
  images: [''], description: '', sizeStock: makeDefaultSizeStock(), highlights: defaultHighlights,
  topwearSizeStock: makeDefaultSizeStock([...APPAREL_SIZES]),
  topwearHighlights: { ...defaultHighlights },
  bottomwearSizeStock: makeDefaultSizeStock([...JEANS_SIZES]),
  bottomwearHighlights: { ...defaultHighlights },
};
...
function ProductModal({
  ...
  const handleSubmit = async () => {
    ...
    const requiresCategory = !isLiveSale && form.section !== 'kurta' && form.section !== 'outfits';
  ...
  return (
    <ModalShell onClose={onClose}>
      ...
      <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setForm(p => ({
                  ...p,
                  section: newSection,
                  // For non-outfit sections: reset regular sizeStock if sizes changed
                  ...(sizeSetChanged && newSection !== 'outfits' ? { sizeStock: makeDefaultSizeStock(newSizes) } : {}),
                  // When switching INTO outfits: reset topwear/bottomwear sizeStocks
                  ...(newSection === 'outfits' && p.section !== 'outfits' ? {
                    topwearSizeStock: makeDefaultSizeStock([...APPAREL_SIZES]),
                    bottomwearSizeStock: makeDefaultSizeStock([...JEANS_SIZES]),
                  } : {}),
                }));
        ...
        {/* Category — hidden for Live Sale, Kurta, and Thunder Looks */}
        {!isLiveSale && form.section !== 'kurta' && form.section !== 'outfits' && (
          <div>
        ...
        {/* Size-based Stock — split into Topwear + Bottomwear for outfits */}
        {form.section === 'outfits' ? (
          <>
            {/* ── Topwear Details ── */}
            <div className="p-4 bg-white/[0.02] border border-white/[0.08] rounded-xl space-y-4">
              <div className="flex items-center gap-2">
                <span className="font-condensed font-bold text-xs tracking-[0.2em] uppercase text-brass">Topwear Details</span>
                <span className="font-condensed text-[0.6rem] text-sv-dim uppercase tracking-widest">(Shirt / T-Shirt)</span>
              </div>
              <div>
                <label className="block font-condensed text-xs text-sv-mid uppercase tracking-wider mb-2">Stock by Size</label>
                <SizeStockInput
                  sizeStock={form.topwearSizeStock}
                  sizes={[...APPAREL_SIZES]}
                  onChange={(ss) => setForm(p => ({ ...p, topwearSizeStock: ss }))}
                />
              </div>
              <div>
                <label className="block font-condensed text-xs text-sv-mid uppercase tracking-wider mb-0.5">Topwear Highlights</label>
                <p className="font-condensed text-[0.65rem] text-sv-mid/50 mb-3">Leave all blank to hide.</p>
                <div className="grid grid-cols-2 gap-3">
                  {([
                    ...
                    <div key={key}>
                      ...
                      <input
                        type="text"
                        value={form.topwearHighlights[key]}
                        onChange={(e) => setForm(p => ({ ...p, topwearHighlights: { ...p.topwearHighlights, [key]: e.target.value } }))}
                        placeholder={label}
            ...
            <div className="p-4 bg-white/[0.02] border border-white/[0.08] rounded-xl space-y-4">
              <div className="flex items-center gap-2">
                <span className="font-condensed font-bold text-xs tracking-[0.2em] uppercase text-brass">Bottomwear Details</span>
                <span className="font-condensed text-[0.6rem] text-sv-dim uppercase tracking-widest">(Jeans / Pants)</span>
              </div>
              <div>
                <label className="block font-condensed text-xs text-sv-mid uppercase tracking-wider mb-2">Stock by Size</label>
                <SizeStockInput
                  sizeStock={form.bottomwearSizeStock}
                  sizes={[...JEANS_SIZES]}
                  onChange={(ss) => setForm(p => ({ ...p, bottomwearSizeStock: ss }))}
                />
              </div>
              <div>
                <label className="block font-condensed text-xs text-sv-mid uppercase tracking-wider mb-0.5">Bottomwear Highlights</label>
                <p className="font-condensed text-[0.65rem] text-sv-mid/50 mb-3">Leave all blank to hide.</p>
                <div className="grid grid-cols-2 gap-3">
                  {([
                    { key: 'color', label: 'Color' },
                    ...
                    <div key={key}>
                      ...
                      <input
                        type="text"
                        value={form.bottomwearHighlights[key]}
                        onChange={(e) => setForm(p => ({ ...p, bottomwearHighlights: { ...p.bottomwearHighlights, [key]: e.target.value } }))}
                        placeholder={label}
                        className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-lg text-tb-white text-sm placeholder:text-sv-mid/30 focus:outline-none focus:border-brass/40 transition-colors"
                      />
                    </div>
        ...
        {/* Product Highlights — hidden for outfits (each piece has its own highlights above) */}
        {form.section !== 'outfits' && (
          <div>
...
export default function Admin() {
  ...
  const [activeTab, setActiveTab] = useState<'analytics' | 'orders' | 'products' | 'categories' | 'reviews' | 'brands' | 'slider' | 'notifications' | 'returns'>('analytics');
  ...
  const [sliderSlides, setSliderSlides] = useState<Array<{ imageUrl: string; heading: string; productId: string; productName: string | null; productImage: string | null }>>([
    { imageUrl: '', heading: 'SHARP', productId: '', productName: null, productImage: null },
    { imageUrl: '', heading: 'REBEL', productId: '', productName: null, productImage: null },
    { imageUrl: '', heading: 'WILD',  productId: '', productName: null, productImage: null },
    { imageUrl: '', heading: 'NOIR',  productId: '', productName: null, productImage: null },
  ]);
  ...
  const [issueRefundError, setIssueRefundError] = useState<Record<string, string>>({});
  ...
  const handleIssueRefund = async (returnId: string) => {
    ...
    try {
      ...
      setIssueRefundError(prev => ({ ...prev, [returnId]: msg }));
    } finally {
  ...
  const fetchReturns = async (silent = false) => {
    ...
    } catch { console.error('Failed to fetch returns'); }
  ...
  const addProduct = async (formData: ProductFormData): Promise<boolean> => {
    ...
    try {
      ...
      const isOutfit = formData.section === 'outfits';
      const images = formData.images.map(s => s.trim()).filter(Boolean);
      const categoryId = (formData.section === 'kurta' || formData.section === 'live-sale' || isOutfit) ? '' : formData.categoryId;

      let bodyPayload: Record<string, unknown>;
      let localStockFields: Record<string, unknown>;

      if (isOutfit) {
        const topwearSS = Object.fromEntries([...APPAREL_SIZES].map(s => [s, Math.max(0, parseInt(formData.topwearSizeStock[s] ?? '0') || 0)]));
        const bottomwearSS = Object.fromEntries([...JEANS_SIZES].map(s => [s, Math.max(0, parseInt(formData.bottomwearSizeStock[s] ?? '0') || 0)]));
        const topStock = Object.values(topwearSS).reduce((sum, qty) => sum + qty, 0);
        const botStock = Object.values(bottomwearSS).reduce((sum, qty) => sum + qty, 0);
        const topHasH = Object.values(formData.topwearHighlights).some(v => v.trim() !== '');
        const botHasH = Object.values(formData.bottomwearHighlights).some(v => v.trim() !== '');
        const topwear = { sizeStock: topwearSS, stock: topStock, highlights: topHasH ? formData.topwearHighlights : null };
        const bottomwear = { sizeStock: bottomwearSS, stock: botStock, highlights: botHasH ? formData.bottomwearHighlights : null };
        const stock = Math.min(topStock, botStock);
        bodyPayload = { name: formData.name, section: formData.section, categoryId, price, mrp, purchasePrice, brandId, images, description: formData.description, topwear, bottomwear };
        localStockFields = { stock, topwear, bottomwear };
      } else {
        const sizes = getSizesForSection(formData.section);
        const sizeStock = Object.fromEntries(sizes.map(s => [s, Math.max(0, parseInt(formData.sizeStock[s] ?? '0') || 0)]));
        ...
        bodyPayload = { name: formData.name, section: formData.section || 'denim', categoryId, price, mrp, purchasePrice, brandId, sizeStock, stock, images, description: formData.description, highlights };
  ...
  const updateProduct = async (formData: ProductFormData): Promise<boolean> => {
    ...
    try {
      ...
      const purchasePrice = formData.purchasePrice ? parseFloat(formData.purchasePrice) : undefined;
      const brandId = formData.brandId || undefined;
      const isOutfitPut = formData.section === 'outfits';
      const images = formData.images.map(s => s.trim()).filter(Boolean);
      const categoryId = (formData.section === 'kurta' || formData.section === 'live-sale' || isOutfitPut) ? '' : formData.categoryId;

      let putBodyPayload: Record<string, unknown>;
      let putLocalStockFields: Record<string, unknown>;
      ...
      if (isOutfitPut) {
        ...
        const bottomwear = { sizeStock: bottomwearSS, stock: botStock, highlights: botHasH ? formData.bottomwearHighlights : null };
        const stock = Math.min(topStock, botStock);
        putBodyPayload = { name: formData.name, section: formData.section, categoryId, price, mrp, purchasePrice, brandId, images, description: formData.description, topwear, bottomwear };
        putLocalStockFields = { stock, topwear, bottomwear };
      } else {
        const sizes = getSizesForSection(formData.section);
        const sizeStock = Object.fromEntries(sizes.map(s => [s, Math.max(0, parseInt(formData.sizeStock[s] ?? '0') || 0)]));
        const stock = Object.values(sizeStock).reduce((sum, qty) => sum + qty, 0);
        ...
        putBodyPayload = { name: formData.name, section: formData.section || 'denim', categoryId, price, mrp, purchasePrice, brandId, sizeStock, stock, images, description: formData.description, highlights: highlightsPut };
      ...
      if (r.ok) {
        setProducts(prev => prev.map(p => p._id === editingProduct._id
          ? {
              ...
              categoryId, price, mrp, purchasePrice, brandId,
              images, image: images[0], description: formData.description, ...putLocalStockFields,
            }
          : p
        ));
  ...
  return (
    <div className="noise-overlay min-h-screen flex flex-col bg-void">
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 md:px-12 lg:px-16">
          ...
          {!loading && (
            <AnimatePresence mode="wait">
              ...
              {/* ── SLIDER ── */}
              {activeTab === 'slider' && (
                <motion.div key="slider" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
                  <div className="flex items-center justify-between mb-6">
                    <div>
                      <h2 className="font-display text-xl sm:text-2xl tracking-[0.06em] text-tb-white uppercase">Thunder Looks Slider</h2>
                      <p className="font-condensed text-xs text-sv-mid mt-1">Configure the 4-slide cinematic carousel shown on the homepage.</p>
                    </div>
                    <button
                      onClick={saveSliderConfig}
                      disabled={sliderSaving}
                      className={`flex items-center gap-1.5 px-4 py-2 font-condensed font-bold text-xs tracking-[0.15em] uppercase rounded-lg transition-all duration-200 ${sliderSaved ? 'bg-green-500/20 border border-green-500/30 text-green-400' : 'bg-tb-white text-void hover:bg-white'} disabled:opacity-50`}
                    >
                      {sliderSaving ? 'Saving…' : sliderSaved ? 'Saved!' : 'Save Slider'}
                    </button>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {sliderSlides.map((slide, i) => (
                      <div key={i} className="bg-white/[0.03] border border-white/10 rounded-xl p-5 space-y-4">
                        <div className="flex items-center justify-between mb-1">
                          <span className="font-condensed font-bold text-xs tracking-[0.2em] uppercase text-brass">Slide {i + 1}</span>
                          {slide.imageUrl && (
                            <img
                              src={slide.imageUrl}
                              alt="Preview"
                        ...
                        <div>
                          <label className="block font-condensed text-xs text-sv-mid uppercase tracking-wider mb-1.5">Heading Word</label>
                          <input
                            type="text"
                            value={slide.heading}
                            maxLength={40}
                            onChange={(e) => setSliderSlides(prev => prev.map((s, j) => j === i ? { ...s, heading: e.target.value.toUpperCase() } : s))}
                            placeholder="e.g. SHARP"
                            className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-tb-white text-sm placeholder:text-sv-mid/40 focus:outline-none focus:border-white/30 transition-colors uppercase"
                          />
                        </div>

                        <div>
                          <label className="block font-condensed text-xs text-sv-mid uppercase tracking-wider mb-1.5">Outfit Image URL</label>
                          <input
                            type="url"
                            value={slide.imageUrl}
                            onChange={(e) => setSliderSlides(prev => prev.map((s, j) => j === i ? { ...s, imageUrl: e.target.value } : s))}
                            placeholder="https://res.cloudinary.com/…"
                            className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-tb-white text-sm placeholder:text-sv-mid/40 focus:outline-none focus:border-white/30 transition-colors"
                          />
                        </div>
                        ...
                        <div>
                          <label className="block font-condensed text-xs text-sv-mid uppercase tracking-wider mb-1.5">
                            Map to Thunder Looks Product
                            <span className="ml-2 normal-case text-sv-dim">(CTA links here)</span>
                          </label>
                          <div className="relative">
                            <select
                              value={slide.productId}
                              onChange={(e) => {
                                const chosen = sliderOutfits.find(p => p._id === e.target.value);
                                setSliderSlides(prev => prev.map((s, j) => j === i ? {
                                  ...s,
                                  productId: e.target.value,
                                  productName: chosen?.name ?? null,
                                  productImage: (chosen as any)?.images?.[0] || chosen?.image || null,
                                } : s));
                              }}
                              className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-tb-white text-sm focus:outline-none focus:border-white/30 transition-colors appearance-none"
                            >
                              <option value="" className="bg-zinc-900 text-sv-mid">— No product mapped —</option>
                              {sliderOutfits.length === 0 ? (
                                <option disabled className="bg-zinc-900 text-sv-mid">No Thunder Looks products yet</option>
                              ) : (
                                sliderOutfits.map(p => (
                                  <option key={p._id} value={p._id} className="bg-zinc-900 text-tb-white">{p.name}</option>
                                ))
                              )}
                            </select>
                            <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-sv-mid pointer-events-none" />
                          ...
                          {slide.productName && (
                            <p className="font-condensed text-xs text-brass mt-1.5">Mapped: {slide.productName}</p>
                          )}
                        </div>
                      </div>
                    ))}
              ...
              {/* ── RETURNS ── */}
              {activeTab === 'returns' && (
                <motion.div key="returns" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: 0.2 }}>
                  <div className="flex items-center justify-between mb-5">
                    <div>
                      <h2 className="font-display text-xl sm:text-2xl tracking-[0.06em] text-tb-white uppercase">Return Requests</h2>
                      <p className="font-condensed text-xs text-sv-mid mt-0.5">Review and approve or reject customer return requests.</p>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="font-condensed text-xs text-amber-400 bg-amber-400/10 border border-amber-400/30 rounded px-2 py-1">
                        {returns.filter(r => r.status === 'pending').length} pending
                      </span>
                      <span className="font-condensed text-xs text-sv-mid bg-white/5 border border-white/10 rounded px-2 py-1">
                  ...
                  {returnsLoading ? (
                    ...
                    <div className="space-y-4">
                      {returns.map((ret) => {
                        const statusColor =
                          ret.status === 'pending'       ? 'text-amber-400 bg-amber-400/10 border-amber-400/30' :
                          ret.status === 'approved'      ? 'text-emerald-400 bg-emerald-400/10 border-emerald-400/30' :
                          ret.status === 'refund_issued' ? 'text-teal-400 bg-teal-400/10 border-teal-400/30' :
                          'text-rose-400 bg-rose-400/10 border-rose-400/30';
                        const StatusIcon =
                          ret.status === 'pending'       ? Clock :
                          ret.status === 'approved'      ? CheckCircle2 :
                          ret.status === 'refund_issued' ? CheckCircle2 :
                          X;
                        ...
                        const shortId = (ret.orderId || '').slice(-8).toUpperCase();

                        return (
                          <div key={ret._id} className="bg-white/[0.03] border border-white/10 rounded-xl p-5">
                            {/* Header row */}
                            ...
                            {ret.status === 'pending' && (
                              <div className="flex gap-3">
                                <button
                                  ...
                                  className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-emerald-500/10 border border-emerald-500/40 rounded-lg text-emerald-400 text-xs font-condensed uppercase tracking-wider hover:bg-emerald-500/20 transition-colors"
                                >
                                ...
                                <button
                                  ...
                                  className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-rose-500/10 border border-rose-500/40 rounded-lg text-rose-400 text-xs font-condensed uppercase tracking-wider hover:bg-rose-500/20 transition-colors"
                                >
                            ...
                            {ret.status === 'approved' && (
                              <div className="space-y-2">
                                ...
                                {issueRefundError[ret._id] && (
                                  <p className="font-condensed text-[10px] text-red-400 bg-red-500/10 border border-red-500/20 rounded px-2.5 py-1.5">
                                    {issueRefundError[ret._id]}
                                  </p>
                                )}
                              </div>
      ...
      <AnimatePresence>
          ...
          <ProductModal
            ...
            initialData={{
              ...
              highlights: editingProduct.highlights
                ...
                : defaultHighlights,
              topwearSizeStock: editingProduct.topwear?.sizeStock
                ? Object.fromEntries(Object.entries(editingProduct.topwear.sizeStock).map(([s, v]) => [s, String(v)]))
                : makeDefaultSizeStock([...APPAREL_SIZES]),
              topwearHighlights: editingProduct.topwear?.highlights
                ? { ...defaultHighlights, ...editingProduct.topwear.highlights }
                : { ...defaultHighlights },
              bottomwearSizeStock: editingProduct.bottomwear?.sizeStock
                ? Object.fromEntries(Object.entries(editingProduct.bottomwear.sizeStock).map(([s, v]) => [s, String(v)]))
                : makeDefaultSizeStock([...JEANS_SIZES]),
              bottomwearHighlights: editingProduct.bottomwear?.highlights
                ? { ...defaultHighlights, ...editingProduct.bottomwear.highlights }
                : { ...defaultHighlights },
            }}
        ...
        {/* Manage Return Modal (approve / reject) */}
        {managingReturn && returnAction && (
          <ModalShell onClose={() => { if (!returnSubmitting) { setManagingReturn(null); setReturnAction(null); setReturnError(''); } }}>
            <div className="px-6 pt-6 pb-2 border-b border-white/10 shrink-0">
              <p className={`font-condensed text-xs uppercase tracking-widest mb-0.5 ${returnAction === 'approve' ? 'text-emerald-400/70' : 'text-rose-400/70'}`}>
                {returnAction === 'approve' ? 'Approve Return' : 'Reject Return'}
              </p>
              <h3 className="font-display text-xl tracking-[0.08em] text-tb-white uppercase pr-8">
                {returnAction === 'approve' ? 'Issue Refund' : 'Decline Request'}
              </h3>
            </div>
            <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
              {/* Return summary */}
              <div className="p-3 bg-white/[0.03] border border-white/8 rounded-lg space-y-1">
                <p className="font-condensed text-xs text-sv-mid">Customer: <span className="text-tb-white">{managingReturn.userId}</span></p>
                <p className="font-condensed text-xs text-sv-mid">Order Total: <span className="text-tb-white">₹{managingReturn.totalAmount.toLocaleString('en-IN')}</span></p>
                <p className="font-condensed text-xs text-sv-mid">Reason: <span className="text-brass">{managingReturn.reason.replace(/_/g, ' ')}</span></p>
              </div>
              ...
              <div>
                <label className="block font-condensed text-xs text-sv-mid uppercase tracking-wider mb-1.5">
                  Admin Notes <span className="text-sv-dim normal-case tracking-normal">(Optional — shown to customer)</span>
                </label>
                <textarea
                  rows={3}
                  value={returnAdminNotes}
                  onChange={(e) => setReturnAdminNotes(e.target.value.slice(0, 500))}
                  placeholder={returnAction === 'approve' ? 'e.g. Approved — refund will be processed in 5 working days.' : 'e.g. Item appears used based on photos provided. Unable to process return.'}
                  className="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-lg text-tb-white text-sm placeholder:text-sv-dim/40 focus:outline-none focus:border-white/30 transition-colors resize-none"
                />
              </div>

              {returnError && (
                <p className="font-condensed text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg px-3 py-2.5">{returnError}</p>
              )}
            </div>
            <div className="px-6 pb-6 pt-3 flex gap-3 shrink-0 border-t border-white/10">
              <button
                onClick={() => { setManagingReturn(null); setReturnAction(null); setReturnError(''); }}
