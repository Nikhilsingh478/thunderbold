...
interface ReturnRequestModalProps {
  ...
  orderNumber?: string;
  totalAmount: number;
  onSubmit: (orderId: string, reason: ReturnReason, description: string) => Promise<void>;
}
...
export default function ReturnRequestModal({
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      await onSubmit(orderId, reason, description.trim());
      setSubmitted(true);
    } catch (err: unknown) {
    ...
    <AnimatePresence>
      {open && (
        ...
        >
          ...
          >
            {/* Header */}
            <div className="px-6 pt-6 pb-4 border-b border-white/10 shrink-0 flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <RotateCcw className="w-4 h-4 text-brass" />
                  <h3 className="font-display text-xl tracking-[0.08em] text-tb-white uppercase">
                    Request Return
                  </h3>
                </div>
                {orderNumber && (
                  <p className="font-condensed text-xs text-sv-mid tracking-wider">
                    Order {orderNumber}
                  </p>
                )}
              </div>
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5 space-y-5">
              {submitted ? (
                ...
                >
                  ...
                  <div>
                    <p className="font-display text-xl tracking-wider text-tb-white uppercase mb-2">
                      Request Submitted
                    </p>
                    <p className="font-condensed text-sm text-sv-mid leading-relaxed max-w-[320px]">
                      We've received your return request. Our team will review it and reach out to you within 2–3 business days.
                    </p>
                  </div>
                  <div className="mt-2 w-full p-4 bg-brass/5 border border-brass/20 rounded-xl">
                    <p className="font-condensed text-xs text-brass uppercase tracking-wider mb-1">
                      Estimated Refund (if approved)
                    </p>
                    <p className="font-display text-2xl tracking-wider text-tb-white">
                      ₹{estimatedRefund.toLocaleString('en-IN')}
                    </p>
                    <p className="font-condensed text-[10px] text-sv-dim mt-1 tracking-wider">
                      Order total ₹{totalAmount.toLocaleString('en-IN')} − ₹{SHIPPING_DEDUCTION} shipping charges
                    </p>
                  </div>
                  <button
                    onClick={handleClose}
                    className="mt-2 px-6 py-2.5 bg-tb-white text-void font-condensed font-bold text-xs tracking-[0.15em] uppercase rounded-lg hover:bg-white transition-colors"
                  >
                    Done
                  </button>
                </motion.div>
                ...
                <>
                  ...
                  <div className="flex items-start gap-3 p-4 bg-amber-500/5 border border-amber-500/20 rounded-xl">
                    ...
                    <p className="font-condensed text-xs text-amber-200/70 leading-relaxed tracking-wide">
                      Returns are accepted for defective, wrong, or significantly misrepresented items.
                      Approved refunds are processed within 5–7 business days, minus ₹{SHIPPING_DEDUCTION} shipping charges.
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="Describe what went wrong — e.g. stitching came apart after first wash, received size 32 instead of ordered 30, colour is very different from photos..."
                  ...
                  <div className="p-4 bg-white/3 border border-white/8 rounded-xl">
                    <p className="font-condensed text-xs text-sv-mid uppercase tracking-wider mb-2">
                      Estimated Refund (if approved)
                    </p>
                    <p className="font-display text-2xl tracking-wider text-tb-white">
                      ₹{estimatedRefund.toLocaleString('en-IN')}
                    ...
                    <p className="font-condensed text-[10px] text-sv-dim mt-1 tracking-wider">
                      ₹{totalAmount.toLocaleString('en-IN')} order total − ₹{SHIPPING_DEDUCTION} shipping charges
                    </p>
                  </div>
                </>
              )}
            ...
            {/* Footer */}
            {!submitted && (
              <div className="px-6 pb-6 pt-4 flex gap-3 shrink-0 border-t border-white/10">
                <button
                  onClick={handleClose}
                  disabled={submitting}
